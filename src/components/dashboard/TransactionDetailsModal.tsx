...
export default function TransactionDetailsModal({ transaction, onClose }: Props) {
  ...
  // FIX: Use 'txStatus' instead of 'status'
  const isSuccess = transaction.txStatus === 'SUCCESS';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className={`px-6 py-8 flex flex-col items-center justify-center ${isSuccess ? 'bg-green-50' : 'bg-red-50'}`}>
            ...
            {transaction.errorMessage && (
                <p className="text-red-600 text-sm mt-2 text-center px-4 font-medium border-t border-red-100 pt-2 w-full">
                    {transaction.errorMessage}
                </p>
            )}
        </div>
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="text-center">
                    <p className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-wider">PAYER</p>
                    <p className="font-bold text-gray-900 text-sm">{transaction.payerPhone}</p>
                    <p className="text-xs text-gray-500 truncate max-w-[100px]">{transaction.clientName}</p>
                </div>
                <ArrowRight className="text-gray-300" />
                <div className="text-center">
                    <p className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-wider">RECEIVER</p>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-3 border border-gray-100 rounded-lg">
                    ...
                    {/* FIX: Use 'paymentMethod' */}
                    <p className="font-semibold text-gray-800 text-sm">{transaction.paymentMethod?.replace('_', ' ') || '-'}</p>
                </div>
                <div className="p-3 border border-gray-100 rounded-lg">
                    <div className="flex items-center gap-2 text-gray-400 text-[10px] font-bold uppercase mb-1">
                ...
                <div className="p-3 border border-gray-100 rounded-lg">
                    ...
                    {/* FIX: Use 'product' instead of 'type' */}
                    <p className="font-semibold text-gray-800 text-sm">{transaction.product?.replace('_', ' ') || '-'}</p>
                </div>
                <div className="p-3 border border-gray-100 rounded-lg">
                    <div className="flex items-center gap-2 text-gray-400 text-[10px] font-bold uppercase mb-1">
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end">
            <button onClick={onClose} className="px-6 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-100 text-gray-700 transition-colors">
                Close
            </button>
        </div>
      </div>
