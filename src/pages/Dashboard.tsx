...
export default function Dashboard() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === 'TRANSACTIONS') {
        ...
        return transactions.filter(item => {
            if (searchTerms) {
                const match = cleanStr(item.clientName).includes(searchTerms) || cleanStr(item.txRef).includes(searchTerms) || cleanStr(item.product).includes(searchTerms);
  ...
  const exportCSV = () => {
    ...
    const headers = isTx 
        ? ["Date", "Ref", "Client", "Product", "Payer", "Receiver", "Amount", "Pay Status", "Tx Status"]
        : ["Date", "Name", "Phone", "Email", "Balance", "Status"];
    
    const rows = filteredData.map((t: any) => isTx 
        ? [new Date(t.date).toLocaleString(), `"${t.txRef}"`, `"${t.clientName}"`, t.product, t.payerPhone, t.receiverPhone, t.amount, t.paymentStatus, t.txStatus]
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm mb-6 no-print">
        <div className="p-4 flex flex-col md:flex-row gap-4">
            <div className="relative flex-1 w-full">
                 ...
                 <input 
                    type="text" 
                    placeholder={activeTab === 'TRANSACTIONS' ? "Search Ref, Client..." : "Search Client Name, Phone..."}
