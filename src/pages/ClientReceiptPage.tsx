...
import ClientReceiptDocument from '../components/ClientReceiptDocument';
import {
  getStoredClientReceipt,
  removeStoredClientReceipt,
...
export default function ClientReceiptPage() {
  ...
  const client = useMemo(
    () => (receiptId ? getStoredClientReceipt(receiptId) : null),
    [receiptId],
  );

  useEffect(() => {
    if (!receiptId || !client) return;
  ...
  useEffect(() => {
    ...
    const cleanup = () => {
      removeStoredClientReceipt(receiptId);
    };
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-100 px-6 py-10">
        <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-lg">
          ...
          <p className="mt-3 text-sm text-slate-500">
            This customer receipt could not be loaded. Please open it again from the dashboard.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-8 print:bg-white print:px-0 print:py-0">
      <div className="mx-auto mb-5 flex w-full max-w-[760px] justify-end print:hidden">
        <button
      ...
      <ClientReceiptDocument data={client} />
    </main>
  );
}
